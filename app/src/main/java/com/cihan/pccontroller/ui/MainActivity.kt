...
import com.cihan.pccontroller.domain.RemoteAction
import com.cihan.pccontroller.domain.RemoteButtonSpec
import com.cihan.pccontroller.edition.EditionSupport
import com.cihan.pccontroller.service.BluetoothHidService
...
class MainActivity : AppCompatActivity() {
    ...
    private val proStore by lazy { ProStore(this) }
    ...
    private lateinit var mouseHelpButton: Button
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            bindViews()
            setupListeners()
            viewModel.refreshPro()
            applyProButtonLabels(viewModel.uiState.value.isPro)
            EditionSupport.start(this, proStore, ::onProUnlocked)
            ...
            Toast.makeText(this, R.string.toast_startup_issue, Toast.LENGTH_LONG).show()
    ...
    private fun onProUnlocked(unlocked: Boolean, showToast: Boolean) {
        runOnUiThread {
            if (!unlocked) return@runOnUiThread
            viewModel.refreshPro()
            applyProButtonLabels(pro = true)
            if (showToast) {
                Toast.makeText(this, R.string.pro_unlocked_toast, Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun bindViews() {
        ...
        mouseHelpButton = findViewById(R.id.mouseHelpButton)
    ...
    private fun setupListeners() {
        ...
        mouseHelpButton.setOnClickListener { showMouseHelpDialog() }
    ...
    private fun openMouseMode() {
        if (!viewModel.uiState.value.isPro) {
            showProUpsell()
            return
        }
        ensureServiceBound()
        if (!viewModel.selectMouseMode()) {
            showProUpsell()
            return
        }
        // Açılışta uyarı yok — trackpad hazır; sorun olursa “Çalışmıyor mu?”
    }

    private fun showMouseHelpDialog() {
        AlertDialog.Builder(this)
            ...
            .setNeutralButton(R.string.mouse_help_probe) { _, _ ->
                testMouseProbe()
            }
    ...
    private fun openKeyboardMode() {
        ...
        ensureServiceBound()
        if (!viewModel.selectKeyboardMode()) {
            showProUpsell()
            return
        }
    ...
    private fun showProUpsell() {
        EditionSupport.showUpsell(
            activity = this,
            proStore = proStore,
            onUnlocked = ::onProUnlocked,
            onDebugUnlock = {
                viewModel.unlockProForDebug()
                applyProButtonLabels(pro = true)
                Toast.makeText(this, R.string.pro_unlocked_toast, Toast.LENGTH_SHORT).show()
            }
        )
    ...
    private fun applyProButtonLabels(pro: Boolean) {
        if (!::openMouseFromPickButton.isInitialized) return
        openMouseFromPickButton.setText(
            if (pro) R.string.mouse_mode else R.string.mouse_mode_locked
        )
        openKeyboardFromPickButton.setText(
            if (pro) R.string.keyboard_mode else R.string.keyboard_mode_locked
        )
        if (::openMouseButton.isInitialized) {
            openMouseButton.setText(
                if (pro) R.string.mouse_mode else R.string.mouse_mode_locked
            )
        }
        if (::openKeyboardButton.isInitialized) {
            openKeyboardButton.setText(
                if (pro) R.string.keyboard_mode else R.string.keyboard_mode_locked
            )
        }
    }
    ...
    private fun keyboardPrefs() =
        getSharedPreferences("pccontroller_keyboard", MODE_PRIVATE)
    ...
    private fun testMouseProbe() {
        window.decorView.postDelayed({
            ...
            if (ok) {
                Toast.makeText(this, R.string.mouse_probe_ok, Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(this, R.string.mouse_probe_fail, Toast.LENGTH_LONG).show()
            }
        }, 400)
    }
    ...
    private fun disconnectAndReset() {
        ...
        connectRequestedAddress = null
        syncingServiceState = false
        hidService?.cancelConnect()
        // Servis henüz bind olmadıysa da ViewModel’i hemen sıfırla
        viewModel.onConnectionState(ConnectionState.Idle)
    ...
    private fun connectTo(address: String) {
        ...
        try {
            ...
            awaitingConnection = false
            connectRequestedAddress = null
            Toast.makeText(this, R.string.toast_connect_failed, Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun render(state: RemoteUiState) {
        when (state.setup) {
            ...
            SetupPhase.NeedBluetoothEnable -> {
                ...
                setStatus(getString(R.string.bt_off_status), connected = false)
    ...
    private fun renderReady(state: RemoteUiState) {
        when (val connection = state.connection) {
            is ConnectionState.Connected -> {
                awaitingConnection = false
                hideAllPanels()
                applyProButtonLabels(state.isPro)
                // Pro kapalıyken mouse/keyboard yüzeyinde kalmasın
                if (!state.isPro &&
                    (state.controlSurface == ControlSurface.Mouse ||
                        state.controlSurface == ControlSurface.Keyboard)
                ) {
                    viewModel.backToModePick()
                    return
                }
            ...
            is ConnectionState.Registered, is ConnectionState.Idle -> {
                if (awaitingConnection && state.selectedAddress != null) {
                    ...
                    cancelConnectButton.visibility = View.VISIBLE
                    setStatus(
                        if (connection is ConnectionState.Registered) R.string.status_connecting
                        else R.string.status_preparing,
                        connected = false
                    )
